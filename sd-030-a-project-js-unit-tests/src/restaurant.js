...
const createMenu = (menu) => ({
        ...
        consumption: [],
        order(order) {
            const food = Object.keys(menu.food);
            const drink = Object.keys(menu.drinks);
            const availableItems = [...food, ...drink];
            const isAvailable = order.every((item) => availableItems.includes(item));
            if (isAvailable) {
        ...
        pay() {
            ...
            this.consumption.forEach((item) => {
                if (Object.prototype.hasOwnProperty.call(menu.food, item)) {
                    sumAll += menu.food[item];
                }
                if (Object.prototype.hasOwnProperty.call(menu.drinks, item)) {
                    sumAll += menu.drinks[item];
                }
            });
            return sumAll * 1.1;
        },
